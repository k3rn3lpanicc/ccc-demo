...
def main():
    ...
    try:
        ...
        if receipt['status'] == 1:
            ...
            if balance_change < 0:
                print(f"   Net change: -{w3.from_wei(abs(balance_change), 'ether')} ETH (gas cost exceeds payout)")
            else:
                print(f"   Net change: +{w3.from_wei(balance_change, 'ether')} ETH (payout minus gas)")
        else:
